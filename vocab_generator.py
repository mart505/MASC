...
class VocabGenerator:
        ...
        for category in categories:
            ...
            with open(f'{self.root_path}/train.txt') as f:
                for line in tqdm(f):
                    flag = False
                    text = line.strip()
                    if category == "mouse-keyboard":
                        if "mouse" in text or "keyboard" in text:
                          flag = True
                    if category in text or flag:
    ...
    def get_from_file():
      ...
      def load_vocab(categories):
        ...
        vocabularies = dict()
        for category in categories:
          f = open(f'{root_path}/dict_{category}.txt', 'r')
          words = []
          for line in f:
      ...
      aspect_categories = aspect_category_mapper[config['domain']]
      sentiment_categories = sentiment_category_mapper[config['domain']]
